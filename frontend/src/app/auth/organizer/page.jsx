...
import { 
  Heart, ArrowRight, Loader2, Calendar, Users, 
  Building, Sparkles, Camera, Mail, Lock, User
} from 'lucide-react';

const features = [
  { icon: Calendar, title: 'Create Weddings', desc: 'Set up multi-day events with detailed schedules' },
  { icon: Users, title: 'Invite Guests', desc: 'Send beautiful invitations and track RSVPs' },
  { icon: Building, title: 'Manage Vendors', desc: 'Coordinate with caterers, photographers & more' },
  { icon: Sparkles, title: 'AI Operations', desc: 'Get intelligent suggestions and assistance' },
  { icon: Camera, title: 'Photo Album', desc: 'Create and share wedding photo galleries' },
];
...
function OrganizerAuthContent() {
  ...
  const { login, signup, isAuthenticated, isOrganizer, user } = useAuth();
  ...
  }, [isAuthenticated, isOrganizer, user, router]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        await signup(formData.email, formData.password, formData.name, 'organizer');
          ...
          >
            ...
            <div className="mt-8 pt-8 border-t border-border text-center">
              <p className="text-sm text-muted-foreground">
                Are you a guest?{' '}
                <Link href="/auth/guest" className="text-primary hover:underline font-medium">
                  RSVP here
                </Link>
              </p>
            </div>
